      ...
      module Pipeline
        class IRHarness
          ...
          def initialize(name = nil, mem_size: Memory::DEFAULT_SIZE, backend: :jit, allow_fallback: true)
            ...
            @inst_mem = Memory.new('inst_mem', size: mem_size)
            @data_mem = Memory.new('data_mem', size: mem_size)
          ...
          def load_program(instructions, start_addr = 0)
            if native_riscv?
              @sim.runner_load_rom(instructions.pack('V*'), start_addr.to_i)
            else
          ...
          def write_pc(value)
            poke_internal_pc!(value & 0xFFFF_FFFF)
          end
          ...
          def poke_internal_pc!(value)
            candidates = %w[pc_reg__pc current_pc pc]
