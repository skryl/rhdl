...
module RISCV
  class Memory < RHDL::HDL::SequentialComponent
    ...
    def self.to_ir(top_name: nil)
      ...
      regs = [
        RHDL::Export::IR::RegArray.new(name: :mem, width: 8, depth: DEFAULT_SIZE)
      ]

      # Internal signals for byte extraction
      nets = [
        RHDL::Export::IR::Net.new(name: :byte0, width: 8),
        RHDL::Export::IR::Net.new(name: :byte1, width: 8),
        RHDL::Export::IR::Net.new(name: :byte2, width: 8),
        RHDL::Export::IR::Net.new(name: :byte3, width: 8),
        RHDL::Export::IR::Net.new(name: :word_data, width: 32),
        RHDL::Export::IR::Net.new(name: :half_data, width: 16)
      ]

      # Byte reads
      assigns = [
        RHDL::Export::IR::Assign.new(target: :byte0, expr: RHDL::Export::IR::MemoryRead.new(
          memory: :mem,
          addr: RHDL::Export::IR::Signal.new(name: :addr, width: 32),
          width: 8
        )),
        RHDL::Export::IR::Assign.new(target: :byte1, expr: RHDL::Export::IR::MemoryRead.new(
          memory: :mem,
          addr: RHDL::Export::IR::BinaryOp.new(
            op: :+,
            left: RHDL::Export::IR::Signal.new(name: :addr, width: 32),
            right: RHDL::Export::IR::Literal.new(value: 1, width: 32),
            width: 32
          ),
          width: 8
        )),
        RHDL::Export::IR::Assign.new(target: :byte2, expr: RHDL::Export::IR::MemoryRead.new(
          memory: :mem,
          addr: RHDL::Export::IR::BinaryOp.new(
            op: :+,
            left: RHDL::Export::IR::Signal.new(name: :addr, width: 32),
            right: RHDL::Export::IR::Literal.new(value: 2, width: 32),
            width: 32
          ),
          width: 8
        )),
        RHDL::Export::IR::Assign.new(target: :byte3, expr: RHDL::Export::IR::MemoryRead.new(
          memory: :mem,
          addr: RHDL::Export::IR::BinaryOp.new(
            op: :+,
            left: RHDL::Export::IR::Signal.new(name: :addr, width: 32),
            right: RHDL::Export::IR::Literal.new(value: 3, width: 32),
            width: 32
          ),
          width: 8
        ))
