...
module RHDL
  module Apple2
    ...
    class IrSimulatorRunner
      ...
      # Nibblized track size expected by Disk II HDL implementation
      TRACK_BYTES = 6656
      ...
      def initialize(backend: :interpret, sub_cycles: 14)
        ...
        # Native backends raise LoadError when unavailable.
        @sim = case backend
               when :interpret
                 RHDL::Sim::Native::IR::Simulator.new(@ir_json, backend: :interpreter, sub_cycles: @sub_cycles)
               when :jit
                 RHDL::Sim::Native::IR::Simulator.new(@ir_json, backend: :jit, sub_cycles: @sub_cycles)
               when :compile
                 RHDL::Sim::Native::IR::Simulator.new(@ir_json, backend: :compiler, sub_cycles: @sub_cycles)
               else
      ...
      def load_disk(path_or_bytes, drive: 0)
        ...
        # Convert DSK to nibblized tracks and load into Disk II controller memory
        tracks = encode_disk(bytes)
        load_disk_tracks_to_controller(tracks)
      ...
      def load_disk_boot_rom
        ...
        return unless File.exist?(boot_rom_path)
        ...
        load_memory_by_name('disk__rom__rom', rom_data, max_len: 256)
      end

      def load_disk_tracks_to_controller(tracks)
        tracks.each_with_index do |track_data, track_num|
          load_memory_by_name('disk__track_memory', track_data, offset: track_num * TRACK_BYTES, max_len: TRACK_BYTES)
        end
      end
      ...
      # Load bytes into a flattened IR memory by name.
      # Requires native IR backend support (see Ir*Wrapper#mem_write_bytes).
      def load_memory_by_name(memory_name, bytes, offset: 0, max_len: nil)
        bytes = bytes.bytes if bytes.is_a?(String)
        bytes = bytes.first(max_len) if max_len

        unless @sim.respond_to?(:get_memory_idx) && @sim.respond_to?(:mem_write_bytes)
          raise "IR backend missing memory load APIs; rebuild native extensions (rake native:build)"
        end

        mem_idx = @sim.get_memory_idx(memory_name)
        raise "Memory not found in IR: #{memory_name}" if mem_idx.nil?

        @sim.mem_write_bytes(mem_idx, offset, bytes)
      end
