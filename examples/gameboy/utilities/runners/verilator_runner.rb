        ...
        cpp_content = <<~CPP
          ...
          unsigned int sim_peek(void* sim, const char* name) {
              ...
              if (strcmp(name, "ext_bus_addr") == 0) return ctx->dut->ext_bus_addr;
	              ...
	              else if (strcmp(name, "gb_core__video_unit__oam_eval") == 0) return
	                  ((ctx->dut->rootp->game_boy_gameboy__DOT__gb_core__DOT__video_unit__DOT__lcdc >> 7) & 0x1) &&
	                  (ctx->dut->rootp->game_boy_gameboy__DOT__gb_core__DOT__video_unit__DOT__v_cnt < 144) &&
	                  (ctx->dut->rootp->game_boy_gameboy__DOT__gb_core__DOT__video_unit__DOT__h_cnt < 20);
          ...
          void sim_run_cycles(void* sim, unsigned int n_cycles, struct GbCycleResult* result) {
              ...
              while (result->cycles_run < n_cycles) {
                  ...
                  ctx->dut->eval();
	                  ...
	                  // Count only effective CPU clocks.
	                  // With SpeedControl parity, CPU advances on debug_clken pulses.
	                  if (ctx->dut->debug_clken) {
	                      ctx->clk_counter++;
	                      result->cycles_run++;
	                  }
