...
unsafe fn runner_write_uart_rx_impl(ctx: *mut JitSimContext, data: *const u8, len: usize) -> usize {
    if ctx.is_null() || data.is_null() || len == 0 {
        return 0;
    }
    let ctx = &mut *ctx;
...
pub unsafe extern "C" fn runner_mem(
    ...
    match (op, space) {
        ...
        (RUNNER_MEM_OP_READ, RUNNER_MEM_SPACE_DISK) => {
            runner_read_disk_impl(ctx as *const JitSimContext, offset, data, len)
        }
        (RUNNER_MEM_OP_READ, RUNNER_MEM_SPACE_UART_TX) => {
            runner_read_uart_tx_impl(ctx as *const JitSimContext, offset, data, len)
        }
        _ => 0,
    }
}
