//! SPARC64 `s1_top` native runner extension for JIT.
//!
...
impl Sparc64Extension {
    ...
    fn apply_inputs(
        ...
        if let Some(response) = response {
            ...
            self.set_signal(core, self.data_i_idx, response.read_data);
        } else {
    ...
    fn sample_request(&self, core: &CoreSimulator) -> Option<Sparc64WishboneRequest> {
        ...
        Some(Sparc64WishboneRequest {
            ...
            addr: canonical_bus_addr(self.signal(core, self.addr_o_idx)),
            data: self.signal(core, self.data_o_idx),
