  ...
  module Synth
    ...
    class Context
      ...
      def to_ir_assigns
        ...
        wire_assigns = @locals.map do |local_var|
          ...
          RHDL::Codegen::Behavior::IR::Assign.new(target: local_var.name, expr: ir_expr)
        end
        ...
        output_assigns = @assignments.map do |assignment|
          ...
          RHDL::Codegen::Behavior::IR::Assign.new(target: assignment[:target], expr: ir_expr)
        end
      ...
      def wire_declarations
        @locals.map do |local_var|
          RHDL::Codegen::Behavior::IR::Net.new(name: local_var.name, width: local_var.width)
        end
      end
    ...
    class Local < Expr
      ...
      def to_ir
        ...
        RHDL::Codegen::Behavior::IR::Signal.new(name: @name, width: @width)
      end
    end
    ...
    class VecAccess < Expr
      ...
      def to_ir
        ...
        result = RHDL::Codegen::Behavior::IR::Signal.new(
        ...
        (count - 2).downto(0) do |i|
          element_signal = RHDL::Codegen::Behavior::IR::Signal.new(
            name: "#{@vec_name}_#{i}",
            width: element_width
          )

          # Condition: index == i
          index_ir = @index.respond_to?(:to_ir) ? @index.to_ir : RHDL::Codegen::Behavior::IR::Signal.new(name: @index.to_s, width: index_width)
          condition = RHDL::Codegen::Behavior::IR::BinaryOp.new(
            op: :==,
            left: index_ir,
            right: RHDL::Codegen::Behavior::IR::Literal.new(value: i, width: index_width),
            width: 1
          )

          result = RHDL::Codegen::Behavior::IR::Mux.new(
            condition: condition,
