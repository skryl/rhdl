    ...
    fn run_mos6502_cycles(&mut self, n: usize) -> usize {
        ...
        for _ in 0..n {
            ...
            if let Some(idx) = clk_list_idx {
                self.prev_clock_values[idx] = 1; // Previous state was high
            }
            ...
            if let Some(idx) = clk_list_idx {
                self.prev_clock_values[idx] = 0; // Previous state was low
            }
...
fn init(ruby: &Ruby) -> Result<(), Error> {
    ...
    // MOS6502 CPU-only mode methods
    class.define_method("mos6502_mode?", method!(RubyJitSim::is_mos6502_mode, 0))?;
    class.define_method("load_mos6502_memory", method!(RubyJitSim::load_mos6502_memory, 3))?;
