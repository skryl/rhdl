...
impl CoreSimulator {
    ...
    pub fn generate_core_code(&self) -> String {
        ...
        // Generate mutable memory arrays if any.
        // These are updated by example-specific extensions that bridge memory writes.
        for (idx, mem) in self.ir.memories.iter().enumerate() {
            let has_non_zero_init = mem.initial_data.iter().any(|&v| v != 0);
            if !has_non_zero_init {
                code.push_str(&format!(
                    "static mut MEM_{}: [u64; {}] = [0u64; {}];\n\n",
                    idx, mem.depth, mem.depth
                ));
                continue;
            }

            let mut init = vec![0u64; mem.depth];
            for (i, &val) in mem.initial_data.iter().take(mem.depth).enumerate() {
                init[i] = val;
            }

            code.push_str(&format!("static mut MEM_{}: [u64; {}] = [\n", idx, mem.depth));
    ...
    pub fn expr_to_rust_ptr(&self, expr: &ExprDef, signals_ptr: &str) -> String {
        match expr {
            ...
            ExprDef::MemRead { memory, addr, width } => {
                ...
                format!(
                    "((if MEM_{}.is_empty() {{ 0u64 }} else {{ let __idx = ({} as usize) % MEM_{}.len(); MEM_{}[__idx] }}) & {})",
                    mem_idx,
                    addr_code,
                    mem_idx,
                    mem_idx,
