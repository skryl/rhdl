...
impl CoreSimulator {
    pub fn new(json: &str) -> Result<Self, String> {
        ...
        // Add combinational assign targets that aren't already in name_to_idx
        // This ensures signals like d1_active (computed from regs) have signal slots
        for assign in &ir.assigns {
            if !name_to_idx.contains_key(&assign.target) {
                let idx = signals.len();
                signals.push(0u64);
                // Get width from expression
                let width = Self::expr_width_static(&assign.expr);
                widths.push(width);
                name_to_idx.insert(assign.target.clone(), idx);
            }
        }
    ...
    pub fn expr_width(&self, expr: &ExprDef) -> usize {
        Self::expr_width_static(expr)
    }

    /// Static version of expr_width that doesn't require self
    fn expr_width_static(expr: &ExprDef) -> usize {
        match expr {
    ...
    pub fn generate_core_code(&self) -> String {
        ...
        let num_mems = self.ir.memories.len();

        // Generate static memory arrays (for backwards compatibility with non-dynamic memory)
        ...
        // Generate evaluate function (inline for performance) - uses static memory
        ...
        // Generate evaluate function with dynamic memory arrays
        if num_mems > 0 {
            code.push_str("/// Evaluate with dynamic memory arrays (for runtime-loaded memory)\n");
            code.push_str("#[inline(always)]\n");
            code.push_str(&format!("pub unsafe fn evaluate_inline_mem(signals: &mut [u64], mems: &[&[u64]; {}]) {{\n", num_mems));
            ...
            for level in &levels {
                for &assign_idx in level {
                    ...
                    if let Some(&idx) = self.name_to_idx.get(&assign.target) {
                        let width = self.widths.get(idx).copied().unwrap_or(64);
                        let expr_code = self.expr_to_rust_mem(&assign.expr);
                        code.push_str(&format!("    signals[{}] = ({}) & {};\n", idx, expr_code, Self::mask_const(width)));
                    }
                }
            }
    ...
    /// Generate Rust code for an expression, using dynamic memory arrays (mems[idx])
    pub fn expr_to_rust_mem(&self, expr: &ExprDef) -> String {
        match expr {
            ...
            ExprDef::UnaryOp { op, operand, width } => {
                let operand_code = self.expr_to_rust_mem(operand);
                match op.as_str() {
            ...
            ExprDef::BinaryOp { op, left, right, width } => {
                let l = self.expr_to_rust_mem(left);
                let r = self.expr_to_rust_mem(right);
            ...
            ExprDef::Mux { condition, when_true, when_false, width } => {
                let cond = self.expr_to_rust_mem(condition);
                let t = self.expr_to_rust_mem(when_true);
                let f = self.expr_to_rust_mem(when_false);
                format!("(if {} != 0 {{ {} }} else {{ {} }} & {})", cond, t, f, Self::mask_const(*width))
            }
            ExprDef::Slice { base, low, width, .. } => {
                let base_code = self.expr_to_rust_mem(base);
            ...
            ExprDef::Concat { parts, width } => {
                ...
                for part in parts.iter().rev() {
                    let part_code = self.expr_to_rust_mem(part);
            ...
            ExprDef::Resize { expr, width } => {
                let expr_code = self.expr_to_rust_mem(expr);
                format!("({} & {})", expr_code, Self::mask_const(*width))
            }
            ExprDef::MemRead { memory, addr, width } => {
                // Use dynamic memory arrays (mems[idx]) instead of static MEM_*
                let mem_idx = self.memory_name_to_idx.get(memory).copied().unwrap_or(0);
                let addr_code = self.expr_to_rust_mem(addr);
                format!("(mems[{}].get({} as usize).copied().unwrap_or(0) & {})",
    ...
    fn generate_tick_function(&self, code: &mut String) {
        ...
        // Generate tick_inline_mem for dynamic memory support (Apple II, etc.)
        self.generate_tick_inline_mem(code);
    }

    /// Generate tick_inline_mem - version that uses dynamic memory arrays
    fn generate_tick_inline_mem(&self, code: &mut String) {
        let clock_indices: Vec<usize> = self.clock_indices.clone();
        let num_clocks = clock_indices.len().max(1);
        let num_regs = self.seq_targets.len();
        let num_mems = self.ir.memories.len();
        ...
        if num_mems == 0 {
            return; // No memories, no need for _mem version
        }

        code.push_str("/// Combined tick with dynamic memory: evaluate_inline_mem + edge-triggered register update\n");
        code.push_str("/// Uses old_clocks (set by caller) for edge detection, not current signal values.\n");
        code.push_str("#[inline(always)]\n");
        code.push_str(&format!("pub unsafe fn tick_inline_mem(signals: &mut [u64], old_clocks: &mut [u64; {}], next_regs: &mut [u64; {}], mems: &[&[u64]; {}]) {{\n",
                               num_clocks, num_regs.max(1), num_mems));

        // Evaluate combinational logic using dynamic memory
        code.push_str("    evaluate_inline_mem(signals, mems);\n\n");

        // Compute next values for all registers using expr_to_rust_mem
        let mut seq_idx = 0;
        for process in &self.ir.processes {
            if !process.clocked {
                continue;
            ...
            for stmt in &process.statements {
                if let Some(&target_idx) = self.name_to_idx.get(&stmt.target) {
                    let width = self.widths.get(target_idx).copied().unwrap_or(64);
                    let expr_code = self.expr_to_rust_mem(&stmt.expr);
                    code.push_str(&format!("    next_regs[{}] = ({}) & {};\n", seq_idx, expr_code, Self::mask_const(width)));
                    seq_idx += 1;
                }
            }
        ...
        // Track which registers have been updated
        code.push_str(&format!("    let mut updated = [false; {}];\n\n", num_regs.max(1)));

        // Check for rising edges using old_clocks vs current signals
        for (domain_idx, &clk_idx) in clock_indices.iter().enumerate() {
            code.push_str(&format!("    // Clock domain {} (signal {})\n", domain_idx, clk_idx));
            code.push_str(&format!("    if old_clocks[{}] == 0 && signals[{}] == 1 {{\n", domain_idx, clk_idx));
        ...
        // Loop to handle derived clocks (with dynamic memory)
        code.push_str("    // Loop for derived clock propagation (with dynamic memory)\n");
        code.push_str("    for _iter in 0..10 {\n");
        code.push_str(&format!("        let mut clock_before = [0u64; {}];\n", num_clocks));
        for (domain_idx, &clk_idx) in clock_indices.iter().enumerate() {
            code.push_str(&format!("        clock_before[{}] = signals[{}];\n", domain_idx, clk_idx));
        ...
        // Final evaluate with dynamic memory
        code.push_str("    evaluate_inline_mem(signals, mems);\n");
