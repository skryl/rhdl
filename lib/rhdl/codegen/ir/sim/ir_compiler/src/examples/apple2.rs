...
impl Apple2Extension {
    ...
    pub fn generate_code(core: &CoreSimulator) -> String {
        ...
        code.push_str("        let d = if cpu_addr >= 0xD000 && cpu_addr < 0x10000 {\n");
        ...
        // Handle RAM write
        code.push_str(&format!("        let ram_addr = (signals[{}] as usize) & 0xFFFF;\n", ram_addr_idx));
        code.push_str(&format!("        let ram_we = signals[{}];\n", ram_we_idx));
        code.push_str("        if ram_we != 0 && ram_addr < ram_len {\n");
        code.push_str(&format!("            ram[ram_addr] = (signals[{}] & 0xFF) as u8;\n", ram_do_idx));
        code.push_str("            // Check for text page write ($0400-$07FF)\n");
        code.push_str("            if ram_addr >= 0x0400 && ram_addr < 0x0800 {\n");
