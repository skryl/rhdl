...
impl GameBoyExtension {
    ...
    pub fn run_gb_cycles(&mut self, core: &mut CoreSimulator, n: usize) -> GbCycleResult {
        ...
        while cycles_run < n {
            ...
            let a15 = Self::peek(core, self.ext_bus_a15_idx) as usize;
            let full_addr = ext_addr | (a15 << 15);

            let cart_wr = Self::peek(core, self.cart_wr_idx);
            if cart_wr != 0 {
                let cart_di = (Self::peek(core, self.cart_di_idx) & 0xFF) as u8;
                if full_addr <= 0x7FFF {
            ...
            Self::poke(core, self.cart_do_idx, value as u64);
            ...
            let _vram_addr_ppu = (Self::peek(core, self.vram_addr_ppu_idx) as usize) & 0x1FFF;
            let zpram_addr = (Self::peek(core, self.zpram_addr_idx) as usize) & 0x7F;
            let wram_addr = (Self::peek(core, self.wram_addr_idx) as usize) & 0x7FFF;
            ...
            let vram_wren_edge = Self::peek(core, self.vram_wren_cpu_idx) != 0;
            let vram_wr_addr = vram_addr_cpu;
            let vram_wr_data = (Self::peek(core, self.cpu_do_idx) & 0xFF) as u8;

            let zpram_wren_edge = Self::peek(core, self.zpram_wren_idx) != 0;
            let zpram_wr_addr = zpram_addr;
            let zpram_wr_data = (Self::peek(core, self.cpu_do_idx) & 0xFF) as u8;

            let wram_wren_edge = Self::peek(core, self.wram_wren_idx) != 0;
            let wram_wr_addr = wram_addr;
            let wram_wr_data = (Self::peek(core, self.cpu_do_idx) & 0xFF) as u8;
            ...
            // Refresh async read outputs after the clock edge.
            let vram_addr_cpu_next = (Self::peek(core, self.vram_addr_cpu_idx) as usize) & 0x1FFF;
            let vram_addr_ppu_next = (Self::peek(core, self.vram_addr_ppu_idx) as usize) & 0x1FFF;
            let zpram_addr_next = (Self::peek(core, self.zpram_addr_idx) as usize) & 0x7F;
            ...
            if lcd_clkena != 0 {
                if self.ppu_pcnt_idx != usize::MAX && self.ppu_v_cnt_idx != usize::MAX {
                    let pcnt = Self::peek(core, self.ppu_pcnt_idx) as usize;
                    let v_cnt = Self::peek(core, self.ppu_v_cnt_idx) as usize;
