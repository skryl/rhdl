...
pub struct VcdTracer {
    ...
    /// Incremental VCD content generated since last drain
    live_chunk: String,
...
impl VcdTracer {
    ...
    fn current_instant() -> Option<Instant> {
        Some(Instant::now())
    }
    ...
    pub fn start(&mut self) {
        self.enabled = true;
        self.time = 0;
        self.start_instant = Self::current_instant();
        self.header_written = false;
        self.live_chunk.clear();
    ...
    pub fn capture(&mut self, signals: &[u64]) {
        ...
        let changes: Vec<SignalChange> = indices_to_update
            .iter()
            .map(|&(idx, val)| {
    ...
    /// Drain live VCD text generated since the last call
    pub fn take_live_chunk(&mut self) -> String {
        std::mem::take(&mut self.live_chunk)
    }
    ...
    fn write_header(&mut self) {
        ...
        self.live_chunk.push_str(&header);
    ...
    fn write_changes(&mut self, changes: &[SignalChange]) {
        if let Some(ref mut writer) = self.file_writer {
            ...
            self.live_chunk.push_str(&output);
        } else {
            let mut output = String::new();
            let mut last_time: Option<u64> = None;
            for change in changes {
