...
const RUNNER_PRESETS = {
  ...
  cpu: {
    ...
    explorerIrPath: './samples/cpu_lib_hdl.json',
    preferredTab: 'vcdTab',
    enableApple2Ui: false,
    usesManualIr: false
  },
...
function setSidebarCollapsed(collapsed) {
  ...
  if (dom.sidebarToggleBtn) {
    dom.sidebarToggleBtn.textContent = state.sidebarCollapsed ? 'Show Config' : 'Hide Config';
...
function updateComponentGraphActivity(cy) {
  ...
  cy.batch(() => {
    cy.nodes('.schem-net').forEach((node) => {
...
function ensureComponentGraph(model) {
  ...
  const graphKey = `${state.components.sourceKey}:schematic:${state.theme}:${focusNode.id}:${showChildren ? 1 : 0}:${focusNode.children.length}:${focusNode.signals.length}`;
  ...
  const schematicPositions = computeComponentSchematicPositions(schematicElements);
  ...
  const cy = window.cytoscape({
    ...
    style: [
      ...
      },
      {
        ...
        style: {
          ...
          'source-arrow-color': palette.wire
        }
      },
      {
    ...
    layout: schematicPositions.size > 0
      ? {
          name: 'preset',
          fit: true,
          padding: 26,
          positions: (node) => schematicPositions.get(node.id()) || { x: 0, y: 0 }
        }
      : {
          name: 'cose',
          animate: false,
...
function renderComponentVisual(node) {
  ...
  const selectedComponentId = (() => {
    ...
    const selected = cy.getElementById(componentCyIdForNode(node.id));
    if (selected && selected.length > 0) {
      return node.id;
    }
    return focusNode?.id || node.id;
  })();
  const selectedCyId = selectedComponentId ? componentCyIdForNode(selectedComponentId) : null;

  cy.batch(() => {
    cy.nodes('.schem-component').removeClass('selected');
    const selected = selectedCyId ? cy.getElementById(selectedCyId) : null;
    if (selected && selected.length > 0) {
...
function renderComponentGraphPanel() {
  ...
  if (dom.componentGraphMeta) {
    ...
    dom.componentGraphMeta.textContent = `selected=${nodeDisplayPath(selectedNode)} | focus=${nodeDisplayPath(focusNode)} | ${mode} | dbl-click component to dive`;
  }
  if (dom.componentGraphFocusPath) {
...
async function start() {
  try {
    ...
    if (startPreset.usesManualIr) {
      ...
      await loadRunnerIrBundle(startPreset, { logLoad: false });
    }
    refreshApple2Screen();
    refreshApple2Debug();
    refreshMemoryView();
