      ...
      class ImportTask < Task
        ...
        def mixed_staged_module_inventory(pure_verilog_files)
          Array(pure_verilog_files).each_with_object({}) do |entry, acc|
            ...
            module_names = parse_verilog_module_names_for_report(path) if module_names.empty?
        ...
        def parse_verilog_module_names_for_report(path)
          parse_verilog_module_names_text_for_report(File.read(path))
        end

        def parse_verilog_module_names_text_for_report(text)
          strip_verilog_comments_for_report(text).scan(/\bmodule\s+([A-Za-z_][A-Za-z0-9_$]*)\b/).flatten.uniq
        end
        ...
        def build_mixed_import_staging(out_dir:)
              ...
              with_timed_step("Synthesize VHDL sources to Verilog (#{synth_targets.length} target(s))") do
                synth_targets.each do |target|
                  ...
                  generated_verilog_entries << {
                    ...
                    declared_modules: parse_verilog_module_names_for_report(out_path)
                  }
          ...
          staged_verilog_sources = staged_source_entries + generated_verilog_entries

          write_staged_verilog_entry(staged_verilog_path: pure_verilog_entry_path, source_files: staged_verilog_sources)
            ...
            vhdl_synth_targets: Array(vhdl_synth_targets).map do |target|
              {
                ...
                .tap { |entry| entry.delete(:source_path) if entry[:source_path].nil? }
            end,
        ...
        def normalize_vhdl_synth_targets(raw)
          ...
          raw.map do |entry|
            ...
            when Hash
              ...
              entry_data.delete(:source_path) if entry_data[:source_path].nil?
              entry_data
            else
        ...
        def write_staged_verilog_entry(staged_verilog_path:, source_files:)
          ...
          source_files.each do |entry|
            path = if entry.is_a?(Hash)
                     entry[:path] || entry[:original_source_path]
                   else
                     entry
                   end
        ...
        def stage_mixed_verilog_files!(verilog_files:, pure_verilog_root:, source_root:, specialization_rewrites: {})
          Array(verilog_files).map do |entry|
            ...
            File.write(staged_path, rewritten)
            {
              ...
              declared_modules: parse_verilog_module_names_text_for_report(rewritten)
            }
          end
        end
        ...
        def runtime_dpram_dif_module_block(module_name)
          ...
          <<~VERILOG
            ...
            module #{module_name}
              ...
              // while only honoring exact 1'b1 on write enables.
              wire enable_a_active = (enable_a !== 1'b0);
              wire enable_b_active = (enable_b !== 1'b0);
              wire cs_a_active = (cs_a !== 1'b0);
              wire cs_b_active = (cs_b !== 1'b0);
        ...
        def postprocess_generated_vhdl_verilog!(entity:, out_path:, module_name: nil)
          ...
          resolved_module_name = module_name.to_s.strip.empty? ? name : module_name.to_s

          rename_generated_module!(out_path: out_path, from: name, to: module_name)
          namespace_generated_helper_modules!(
            out_path: out_path,
            primary_module_name: resolved_module_name
          )
        ...
        def overlay_runtime_generated_vhdl_modules!(pure_verilog_root:)
          generated_dir = pure_verilog_root && File.join(pure_verilog_root, 'generated_vhdl')
          return [] unless generated_dir && Dir.exist?(generated_dir)
          ...
          Dir.glob(File.join(generated_dir, '**', '*.v')).sort.each do |path|
            ...
            replaced << module_name
          end

          unless replaced.empty?
            puts "Patched staged Verilog runtime modules: #{replaced.join(', ')}"
          end
        ...
        def restore_generated_t80_di_reg_alias!(out_path:)
          source = File.read(out_path)
          updated = source.gsub(/module\b.*?endmodule/m) do |module_block|
            next module_block unless module_block.match?(/\bwire\s+\[7:0\]\s+di_reg\s*;/)
            next module_block if module_block.match?(/\bassign\s+di_reg\s*=/)

            port_name =
              if module_block.match?(/\binput\s+(?:\[[^\]]+\]\s*)?DI\b/)
                'DI'
              elsif module_block.match?(/\binput\s+(?:\[[^\]]+\]\s*)?di\b/)
                'di'
              end
            next module_block if port_name.nil?

            module_block.sub(
              /^(\s*)wire\s+\[7:0\]\s+di_reg\s*;\s*$/m,
              "\\0\n\\1assign di_reg = #{port_name}; // restored T80 DI_Reg <= DI alias"
            )
          end
          return if updated == source
