...
RSpec.describe 'Gate-level backend equivalence' do
  ...
  it 'matches full adder outputs' do
    ...
    sim = RHDL::Sim.gate_level([adder], backend: :interpreter, lanes: lanes, name: 'full_adder')
  ...
  it 'matches ripple adder outputs' do
    ...
    sim = RHDL::Sim.gate_level([adder], backend: :interpreter, lanes: lanes, name: 'ripple_adder')
  ...
  it 'matches register outputs over cycles' do
    ...
    sim = RHDL::Sim.gate_level(gate_dffs, backend: :interpreter, lanes: lanes, name: 'register')
  ...
  it 'matches muxed datapath outputs over cycles' do
    ...
    sim = RHDL::Sim.gate_level([mux, adder, dff], backend: :interpreter, lanes: lanes, name: 'muxed_path')
  ...
  it 'has a GPU backend parity stub when enabled' do
    ...
    skip 'GPU backend not available' unless RHDL::Codegen::Netlist::SimGPU.available?

    adder = RHDL::HDL::FullAdder.new('fa')
    gpu_sim = RHDL::Sim.gate_level([adder], backend: :gpu, lanes: lanes, name: 'gpu_parity')
    cpu_sim = RHDL::Sim.gate_level([adder], backend: :cpu, lanes: lanes, name: 'cpu_parity')
...
RSpec.describe 'Netlist Simulator Modes', :slow do
  ...
  def create_simulator(backend, components, name)
    RHDL::Sim.gate_level(components, backend: backend, lanes: lanes, name: name)
