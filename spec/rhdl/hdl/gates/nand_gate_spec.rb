...
RSpec.describe RHDL::HDL::NandGate do
  ...
  describe 'synthesis' do
    ...
    it 'generates valid CIRCT MLIR' do
      mlir = RHDL::HDL::NandGate.to_circt
      expect(mlir).to include('hw.output')
      expect(mlir).to include('hw.module @nand_gate')
    ...
    context 'CIRCT firtool validation', if: HdlToolchain.firtool_available? && HdlToolchain.iverilog_available? do
      it 'CIRCT-generated Verilog matches RHDL Verilog behavior' do
        ...
        result = CirctHelper.validate_circt_export(
          ...
          base_dir: 'tmp/circt_test/nand_gate'
        )
  ...
  describe 'gate-level netlist' do
    ...
    let(:ir) { RHDL::Codegen::Netlist::Lower.from_components([component], name: 'nand_gate') }
    ...
    describe 'simulator comparison' do
      it 'all simulators produce matching results' do
        ...
        NetlistHelper.compare_and_validate!(
          ...
          'nand_gate',
          test_cases,
          base_dir: 'tmp/netlist_comparison/nand_gate',
          has_clock: false
        )
      end
    end
