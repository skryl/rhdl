...
RSpec.describe RHDL::CLI::Tasks::WebGenerateTask do
  ...
  describe '#build_wasm_backends' do
    it 'builds dedicated compiler AOT artifacts for apple2, cpu8bit, mos6502, gameboy, and riscv' do
      task = described_class.new
      ...
      expect(task).to receive(:build_compiler_aot_wasm).with(
        ir_path: described_class::GAMEBOY_AOT_IR_PATH,
        artifact: 'ir_compiler_gameboy.wasm'
      )
      expect(task).to receive(:build_compiler_aot_wasm).with(
