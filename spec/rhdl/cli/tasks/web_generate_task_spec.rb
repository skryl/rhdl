  ...
  describe '#write_mruby_emscripten_config' do
    it 'writes emscripten config that embeds rhdl sources into wasm fs' do
      ...
      expect(File).to receive(:write) do |path, content|
        ...
        expect(content).to include("conf.linker.flags << '-sASYNCIFY=1'")
        expect(content).to include("conf.linker.flags << %q{-sASYNCIFY_IMPORTS=['fd_read']}")
        expect(content).to include("conf.linker.flags << '-sASYNCIFY_STACK_SIZE=131072'")
