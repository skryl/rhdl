...
native_class = native_available ? RHDL::Sim::Native::Netlist::Interpreter : Object
...
RSpec.describe native_class, if: native_available do
  ...
  describe 'integration with HDL components' do
    it 'simulates a NOT gate from HDL' do
      ...
      ir = RHDL::Codegen::Netlist::Lower.from_components([component], name: 'not_test')
    ...
    it 'simulates an AND gate from HDL' do
      ...
      ir = RHDL::Codegen::Netlist::Lower.from_components([component], name: 'and_test')
      sim = described_class.new(ir.to_json, 64)
  ...
  describe 'equivalence with Ruby SimCPU' do
    ...
    it 'produces same results for sequential logic' do
      ...
      ir = RHDL::Codegen::Netlist::IR.new(name: 'dff_test')
      2.times { ir.new_net }
      ir.add_input('d', [0])
      ir.add_output('q', [1])
      ir.add_dff(d: 0, q: 1)
...
RSpec.describe RHDL::Sim::Native::Netlist::Simulator do
  let(:ir) do
  ...
  describe 'when native is available', if: RHDL::Sim::Native::Netlist::INTERPRETER_AVAILABLE do
  ...
  describe 'unified simulator behavior',
           if: (RHDL::Sim::Native::Netlist::INTERPRETER_AVAILABLE ||
                ...
                RHDL::Sim::Native::Netlist::COMPILER_AVAILABLE) do
    let(:backend) do
      if RHDL::Sim::Native::Netlist::INTERPRETER_AVAILABLE
        :interpreter
      elsif RHDL::Sim::Native::Netlist::JIT_AVAILABLE
        :jit
      else
        :compiler
      end
