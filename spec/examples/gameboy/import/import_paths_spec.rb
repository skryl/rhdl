  ...
  it 'emits stable mixed staging/report paths and strict diagnostics', timeout: 1800 do
    ...
    Dir.mktmpdir('gameboy_import_paths_out') do |out_dir|
      Dir.mktmpdir('gameboy_import_paths_ws') do |workspace|
        ...
        expect(artifacts.fetch('runtime_json_path')).to eq(runtime_json)
        expect(artifacts.fetch('firtool_verilog_path')).to eq(firtool_verilog)
        expect(artifacts.fetch('normalized_verilog_path')).to eq(normalized_verilog)
        expect(mixed.fetch('workspace_runtime_json_path')).to eq(workspace_runtime_json)
        expect(mixed.fetch('workspace_firtool_verilog_path')).to eq(workspace_firtool_verilog)
