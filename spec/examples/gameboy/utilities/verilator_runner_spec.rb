  ...
  describe '#resolve_direct_verilog_source_plan' do
    it 'uses normalized imported verilog and builds a generated wrapper top' do
      Dir.mktmpdir('rhdl_gb_direct_verilog') do |dir|
        ...
        expect(plan[:wrapper_source]).to include('.cart_oe(cart_oe)')
        expect(plan[:wrapper_source]).to include('.cart_ram_size(cart_ram_size)')
        ...
        expect(plan[:port_declarations]).to include(
          ...
          include(direction: :in, name: 'cart_oe', width: 1),
          include(direction: :in, name: 'cart_ram_size', width: 8),
