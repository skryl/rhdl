...
RSpec.describe RHDL::HDL::CPU::FastHarness do
  ...
  before do
    ...
    allow(described_class).to receive(:ensure_synth_to_gpu_available!).and_return(true)
  ...
  it 'uses runner-backed memory in synth_to_gpu mode' do
    harness = described_class.new(nil, sim: :synth_to_gpu)
    ...
    expect(harness.backend).to eq(:synth_to_gpu)
    expect(harness.native?).to be(true)
  end

  it 'requires runner-capable cpu8bit mode for synth_to_gpu' do
    allow(sim).to receive(:runner_mode?).and_return(false)

    expect { described_class.new(nil, sim: :synth_to_gpu) }
      .to raise_error(ArgumentError, /runner mode/i)
  end

  it 'surfaces SynthToGPU capability failures clearly' do
    allow(described_class).to receive(:ensure_synth_to_gpu_available!)
      .and_raise(ArgumentError, 'synth_to_gpu toolchain unavailable')

    expect { described_class.new(nil, sim: :synth_to_gpu) }
      .to raise_error(ArgumentError, /synth_to_gpu toolchain unavailable/)
  end
end
