...
RSpec.describe MOS6502::DataLatch do
    ...
    context 'when iverilog is available', if: HdlToolchain.iverilog_available? do
      it 'behavior Verilog compiles and runs' do
        ...
        result = NetlistHelper.run_behavior_simulation(
          ...
          base_dir: 'tmp/behavior_test/mos6502_data_latch',
          has_clock: true
        )
  ...
  describe 'gate-level netlist' do
    ...
    let(:ir) { RHDL::Export::Structure::Lower.from_components([component], name: 'mos6502_data_latch') }
    ...
    context 'when iverilog is available', if: HdlToolchain.iverilog_available? do
      it 'compiles and simulates structure Verilog' do
        ...
        result = NetlistHelper.run_structure_simulation(ir, vectors, base_dir: 'tmp/netlist_test/mos6502_data_latch')
        expect(result[:success]).to be(true), result[:error]
      end
    end
  end
