...
RSpec.describe 'AO486 default patch-series trace import surface' do
  ...
  def firtool_accepts?(mlir_text)
    ...
    Dir.mktmpdir('ao486_trace_patch_series_firtool') do |dir|
  ...
  def export_verilog(mlir_text)
    ...
    Dir.mktmpdir('ao486_trace_patch_series_export') do |dir|
      in_path = File.join(dir, 'input.mlir')
      out_path = File.join(dir, 'output.v')
      File.write(in_path, mlir_text)
  ...
  it 'adds stable trace ports through the default AO486 patch series at import time', timeout: 240 do
    require_import_tool!
    skip 'circt-opt not available' unless HdlToolchain.which('circt-opt')

    Dir.mktmpdir('ao486_trace_patch_series_out') do |out_dir|
      Dir.mktmpdir('ao486_trace_patch_series_ws') do |workspace|
  ...
  it 'exports the default AO486 patch series through firtool', timeout: 240 do
    require_import_tool!
    skip 'circt-opt not available' unless HdlToolchain.which('circt-opt')
