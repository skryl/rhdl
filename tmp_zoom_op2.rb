...
cycle=0; press=20_000_000; start=20_294_483; stop=20_294_490
while cycle < start
...
while cycle<stop
  ...
  puts "cy=#{cycle} t=#{rc.call('gb_core__cpu__debug_t_state')}\/#{rv.call('debug_t_state')} m=#{rc.call('gb_core__cpu__debug_m_cycle')}\/#{rv.call('debug_m_cycle')} ir=#{'%02X'% (rc.call('gb_core__cpu__debug_ir')&0xFF)}\/#{'%02X'% (rv.call('debug_ir')&0xFF)} save=#{rc.call('gb_core__cpu__debug_save_alu')}\/#{rv.call('debug_save_alu')} op=#{rc.call('gb_core__cpu__debug_alu_op')}\/#{rv.call('debug_alu_op')} flags=#{'%02X'% (rc.call('gb_core__cpu__debug_alu_flags')&0xFF)}\/#{'%02X'% (rv.call('debug_alu_flags')&0xFF)} f=#{'%02X'% (c.cpu_state[:f]&0xFF)}\/#{'%02X'% (v.cpu_state[:f]&0xFF)} busa=#{'%02X'%(rc.call('gb_core__cpu__debug_bus_a')&0xFF)}\/#{'%02X'%(rv.call('debug_bus_a')&0xFF)} busb=#{'%02X'%(rc.call('gb_core__cpu__debug_bus_b')&0xFF)}\/#{'%02X'%(rv.call('debug_bus_b')&0xFF)}"
end
