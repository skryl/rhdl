...
import { BACKEND_DEFS, getBackendDef } from '../../../../app/components/sim/runtime/backend_defs.mjs';
...
test('getBackendDef resolves known backends and falls back to interpreter', () => {
  ...
  assert.equal(getBackendDef('arcilator').id, 'arcilator');
...
test('arcilator backend has CIRCT-specific properties', () => {
  const def = BACKEND_DEFS.arcilator;
  assert.ok(def);
  assert.equal(def.id, 'arcilator');
  assert.equal(def.label, 'Arcilator (CIRCT)');
  assert.ok(def.wasmPath.includes('arcilator'));
  assert.equal(def.createFn, 'sim_create');
  assert.equal(def.destroyFn, 'sim_destroy');
});
