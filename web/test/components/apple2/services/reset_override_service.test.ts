...
import { createApple2ResetOverrideService } from '../../../../app/components/apple2/services/reset_override_service';
...
test('apple2 reset override service validates reset vector format', async () => {
  ...
  const service = createApple2ResetOverrideService({
    ...
    parsePcLiteral: () => null,
    hexWord: (value: unknown) => Number(value).toString(16).toUpperCase().padStart(4, '0'),
...
test('apple2 reset override service applies vector and updates memory pane', async () => {
  ...
  const service = createApple2ResetOverrideService({
    dom,
    setMemoryFollowPcState: (value: boolean) => {
      followState = value;
    },
