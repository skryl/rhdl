...
${err.stack}`;
  ...
  function hasSharedArrayBufferSupport() {
    return typeof globalObject.SharedArrayBuffer === "function" && typeof globalObject.Atomics?.wait === "function";
  }
  function initializeStdinChannel(bufferBytes = DEFAULT_STDIN_BUFFER_BYTES) {
    const capacity = Number.isFinite(bufferBytes) && bufferBytes > 1024 ? Math.floor(bufferBytes) : DEFAULT_STDIN_BUFFER_BYTES;
  ...
  function emitStreamChunk(type, value) {
    ...
    if (typeof value === "number") {
      chunk = String.fromCharCode(value & 255);
    } else {
      chunk = `${String(value ?? "")}
`;
    }
  ...
  function startMirbProcess() {
    ...
    const callMain = typeof globalObject.callMain === "function" ? globalObject.callMain : globalObject.Module?.callMain;
    ...
    processStartPromise = Promise.resolve().then(() => callMain([])).then((exitValue) => {
      processExitCode = Number.isFinite(exitValue) ? Number(exitValue) : 0;
      processExited = true;
      processRunning = false;
      processStartPromise = null;
      closeStdinChannel();
      const message = runtimeAbortReason || `mirb process exited (code ${processExitCode})`;
      ...
      processStartPromise = null;
      closeStdinChannel();
      const message = runtimeAbortReason || `mirb process failed: ${asMessage(err)}`;
  ...
  function handleMessage(event) {
    ...
    if (type === "init") {
      initializeWorker(payload).catch((err) => {
        globalObject.postMessage({
          type: "init_error",
