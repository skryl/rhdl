...
export function createApple2MemoryController({
  ...
  function readApple2MappedMemory(start, length) {
    ...
    while (cursor < len) {
      const span = Math.min(len - cursor, addressSpace - addr);
      const chunk = runtime.sim.apple2_read_memory(addr, span);
  ...
  function refreshMemoryView() {
    ...
    if (!isApple2UiEnabled()) {
      renderMemoryPanel(dom, {
        followDisabled: true,
        followChecked: !!state.memory.followPc,
        dumpText: 'Load the Apple II runner to browse memory.',
        disasmText: 'Load the Apple II runner to view disassembly.'
      });
    ...
    if (state.memory.followPc && pc != null) {
      const maxStart = Math.max(0, addressSpace - length);
    ...
    for (let i = 0; i < data.length; i += 16) {
      ...
      const hasPc = pc != null && (((pc & 0xffff) - addr + addressSpace) & 0xffff) < row.length;
      const marker = hasPc ? '>>' : '  ';
