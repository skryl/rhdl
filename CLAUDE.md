...
│   └── tasks/                   # Rake task definitions (*.rake files)
│       ├── dev.rake             # Development tasks (spec, setup, benchmark)
│       ├── diagrams.rake        # Diagram generation tasks
│       ├── hdl.rake             # HDL export tasks
│       ├── gates.rake           # Gate-level synthesis tasks
│       ├── tui.rake             # TUI debugger tasks
│       ├── apple2.rake          # Apple II emulator tasks
│       ├── native.rake          # Native extension tasks
│       └── combined.rake        # Combined/aggregate tasks
│
...
The project includes rake tasks for common operations. Tasks are organized into namespaces:
- `dev:` - Development tasks (tests, setup, benchmarks)
- `cli:` - CLI tasks (diagrams, hdl, gates, tui, apple2)
- `native:` - Native extension tasks
...
rake cli:diagrams:clean

# Export DSL components to Verilog
rake cli:hdl:export

# Export Verilog
rake cli:hdl:verilog
...
**All rake tasks must live in `cli/tasks/*.rake` files.** The `Rakefile` only loads these files; it should not contain task definitions directly.

**Task file organization:**
- `cli/tasks/dev.rake` - Development tasks (spec, setup, parallel, deps, benchmark)
- `cli/tasks/diagrams.rake` - Diagram generation (`cli:diagrams:*`)
- `cli/tasks/hdl.rake` - HDL export (`cli:hdl:*`)
- `cli/tasks/gates.rake` - Gate-level synthesis (`cli:gates:*`)
- `cli/tasks/tui.rake` - TUI debugger (`cli:tui:*`)
- `cli/tasks/apple2.rake` - Apple II emulator (`cli:apple2:*`)
- `cli/tasks/native.rake` - Native extension build (`native:*`)
- `cli/tasks/combined.rake` - Aggregate tasks (`cli:generate_all`, etc.)

**Adding a new rake task:**
1. Create or edit the appropriate `.rake` file in `cli/tasks/`
2. Use proper namespacing (e.g., `dev:`, `cli:`, `native:`)
3. Include a description using `desc` so it appears in `rake -T`
4. Use `load_cli_tasks` to access task implementation classes from `lib/rhdl/cli/tasks/`

**Example:**
```ruby
# cli/tasks/my_feature.rake
namespace :cli do
  namespace :my_feature do
    desc "[CLI] Run my feature task"
    task :run do
      load_cli_tasks
      RHDL::CLI::Tasks::MyFeatureTask.new.run
    end
  end
end
```
