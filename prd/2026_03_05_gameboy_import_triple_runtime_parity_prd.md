...
Wrapper runtime update - the strengthened POP/logo Verilator parity work exposed that the handwritten `Gameboy` wrapper path was still using the legacy `SpeedControl` simplification (`ce`, `ce_n`, `ce_2x` all tied high) instead of the reference 8-phase waveform. Direct Verilator probes showed the wrapper staying inside the early DMG boot ROM VRAM-clear loop and never reaching cartridge fetch or nonblank video. The wrapper parity path is now being corrected to use the shared reference waveform before the stronger logo/framebuffer assertions are re-enabled. (2026-03-10)
Import wrapper update - mixed import now emits an import-local `gameboy.rb` wrapper top and records it in `import_report.json`. Imported runner resolution for custom HDL trees now prefers that generated wrapper and no longer falls back to handwritten `examples/gameboy/hdl/gameboy.rb` / `examples/gameboy/hdl/speedcontrol.rb`. Fresh direct import smoke on March 10, 2026 confirms a real imported output resolves `Gameboy` as the selected wrapper top with Verilog module name `gameboy`. (2026-03-10)
Wrapper parity update - the runnable Game Boy import specs now build their Verilator legs through a generated `gameboy` wrapper top instead of compiling the bare `gb` core directly. Fresh local validation on March 10, 2026 shows the wrapper-backed default behavioral parity gate is green, the report/layout import specs are green, and the strengthened POP-ROM triple-Verilator parity spec is green at a reduced `RHDL_GAMEBOY_VERILATOR_PARITY_MAX_CYCLES=5000000` probe budget. (2026-03-10)
